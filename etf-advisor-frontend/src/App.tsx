// Root component. Shows login page if not authenticated, otherwise
// the main dashboard with navigation between Markets, Watchlist, Admin, Profile.

import { useCallback, useEffect, useState } from 'react'
import type { Etf, WatchlistEntry } from './types'
import { getEtfs, getWatchlist, setApiToken } from './api/etfAdvisorApi'
...
type View = 'markets' | 'watchlist' | 'admin' | 'profile'
...
export default function App() {
  ...
  const loadEtfs = useCallback(() => {
    setLoading(true)
    getEtfs()
        .then((data) => { setEtfs(data); setError(null) })
  ...
  }, [auth.isLoggedIn, loadEtfs, loadWatchlist])

  // Sync token on login/logout
  useEffect(() => {
      ...
      <div className="app">
        <header className="header">
          ...
          <nav className="nav">
            ...
            {auth.isAdmin && (
                <button
                    className={`nav__item ${view === 'admin' ? 'nav__item--active' : ''}`}
            ...
            <button
                className={`nav__item ${view === 'profile' ? 'nav__item--active' : ''}`}
        ...
        {!loading && !error && view === 'markets' && (
            <>
              {showForm && auth.isAdmin && (
                  <EtfForm onCreated={() => { loadEtfs(); setShowForm(false) }} />
              )}
        ...
        {!loading && !error && view === 'admin' && auth.isAdmin && (
            <main className="layout-single">
              <AdminPanel onEtfsChanged={loadEtfs} />
            </main>
        )}
